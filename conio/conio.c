...
void textmode(int new_mode) {
	static int lastmode = _DEFAULTMODE;
	if(new_mode == -1) 
		new_mode=lastmode;
	else
		lastmode=new_mode;
	_setvideomode((short)(new_mode));
}
void  gettextinfo (struct text_info *r ) {
	struct videoconfig vc;
	struct rccoord rc;
	_gettextwindow(&(r->wintop),&(r->winleft),&(r->winbottom),&(r->winright));
	rc = _gettextposition();
	r->cury = rc.row;
	r->curx = rc.col;
	r->attribute = r->normattr = _gettextcolor();
	_getvideoconfig(&vc);
	r->screenheight = vc.numtextrows;
	r->screenwidth = vc.numtextcols;
	r->currmode = vc.mode;
}
void   window( int left, int top, int right, int bottom) {
	_settextwindow((short)top,(short)left,(short)right,(short)bottom);
}
#else
void clrscr(void)
{
