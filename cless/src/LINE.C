...
{
	register int w;
	static short flag_half_wide_char = 0;
	if(curr==0) 
		flag_half_wide_char=0;
		
	if(c<=127) {
		flag_half_wide_char=0;
	}
	else if(flag_half_wide_char>0) {
		flag_half_wide_char=0;
	}
	else {
		flag_half_wide_char=1;
	}
//	printf("%d",flag_half_wide_char);
	w = pwidth(c, a);
	if(flag_half_wide_char>0 && column + w + attr_ewidth(a) >=sc_width) {
