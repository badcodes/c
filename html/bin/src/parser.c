...
int main(int argc,char** argv) 
{
    ...
    while (1) {
        static struct option long_options[]={
            {"baseurl",optional_argument,0,'b'},
            {"help",no_argument,0,'h'},
            {0,0,0,0}
        };
        int opt_idx=0;
        int c = getopt_long(argc,argv,"hb::",long_options,&opt_idx);
        if (c==-1) break;
        switch(c){
            case 'b':
                if (optarg) baseurl=strdup(optarg);
                break;
            case 'h':
                printf("-h,--help\t\tDisplay this.\n");
                printf("-b,--baseurl [URL]\tSet base url.\n");
                printf("Example:cat index.html | htmllink -b \"www.google.com\"\n");
                return 0;
        }
    }

    if (optind<argc && !baseurl)
        baseurl=argv[optind];
